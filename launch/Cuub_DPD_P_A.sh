...
if (( ${#TIME_LIST[@]} == 1 )); then
	CUU_TIME=Cuub_I$(float_to_letters $INITIAL_FRAME)_T$(float_to_letters $TIME)_M$(float_to_letters $INTERVAL_MAXIMUM)_C$(float_to_letters $N_CASES)
else
	CUU_TIME=Cuub_I$(float_to_letters $INITIAL_FRAME)_M$(float_to_letters $INTERVAL_MAXIMUM)_C$(float_to_letters $N_CASES)
fi
...
for TIME in $TIME; do
	export TIME=\$TIME
	/home/yketa/bin/_colmig_DPD_P_A_Cuub >> $output_file
done
EOF
