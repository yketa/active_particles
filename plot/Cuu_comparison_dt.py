...
Ncases = int(eval(os.environ['N_CASES'])) if 'N_CASES' in os.environ else 1000 # number of cases for Cuu calculation
init_frame = int(eval(os.environ['INITIAL_FRAME'])) if 'INITIAL_FRAME' in os.environ else 5000 # initial frame in Cuu calculation
int_max = int(eval(os.environ['INTERVAL_MAXIMUM'])) if 'INTERVAL_MAXIMUM' in os.environ else 1 # maximum number of interval in Cuu calculation

snip0 = str('Cuu' + ('b' if not('ENDPOINT' in os.environ and eval(os.environ['ENDPOINT'])) else ''))
snip1 = 'I%s' % float_to_letters(init_frame)
snip2 = 'M%s_C%s.pickle' % tuple(map(float_to_letters, [int_max, Ncases]))
files = [file for file in os.listdir() if (snip0 in file) and (snip1 in file) and (snip2 in file)] # Cuu pickle files
...
points_x = int(eval(os.environ['POINTS_THETA'])) if 'POINTS_THETA' in os.environ else 10 # number of radii to evaluate for fitting line
...
Cuu = {} # Cuu1D with renormalisation by Cnn
for file in files:
	with open(file, 'rb') as Cuu_file:
		Cuu[file] = pickle.load(Cuu_file)[2]
...
for file in files:
	ax.loglog(Cuu[file][:, 0]/av_p_sep, Cuu[file][:, 1], color=colors[dt(file)], label=r'$\Delta t = %.0e$' % (period_dump*time_step*dt(file)))
...
legend_slope = lambda sl, law: Line2D([0], [0], color='black', linestyle='--', label=r'$C_{uu}(r) \propto$' + {'Powerlaw': r'$r^{%.2e}$' % sl, 'Exponential': r'$e^{%.2er}$' % sl}[law]) # legend for adjustable powerlaw line
...
title += '\n'
title += r'$S_{init}=%.2e, S_{max}=%.2e, N_{cases}=%.2e$' % (init_frame, int_max, Ncases)
fig.suptitle(title)
