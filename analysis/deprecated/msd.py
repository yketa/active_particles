...
def mean_square_displacement(data_dir='/home/yketa/hoomd', param_file='param.pickle', pos_file='position.csv', init_frame=-1, snap_max=1, snap_period=1):
	...
	for dt in intervals(Nframes, Ntimes): # loop over all different increment of time
		# CALCULATION
		time = time_step*period_dump*dt # time at which is evaluated the mean square displacement
		msdis, stdev = (lambda val: (np.mean(val), np.std(val)))([np.sum(((lambda positions: positions - np.mean(positions, axis=0))(pos(time + dt)) - (lambda positions: positions - np.mean(positions, axis=0))(pos(time)))**2, axis=1) for time in times(Nframes, dt)])
		# OUTPUT
		msd_dump.write(str("%e,%e,%e\n" % (time, msdis, stdev/np.sqrt(N*len(times(Nframes, dt))))))
