...
def mean_square_displacement(data_dir='/home/yketa/hoomd', param_file='param.pickle', pos_file='position.csv', init_frame=-1, snap_max=10, snap_period=1):
	...
	out_file = data_dir + '/msd_D' + float_to_letters(density) + '_V' + float_to_letters(vzero) + '_R' + float_to_letters(dr) + '_N' + float_to_letters(N) + '_I' + float_to_letters(init_frame) + '_M' + float_to_letters(snap_max) + '_P' + float_to_letters(snap_period) + '.csv' # MSD file
